...
| ID | 状态 | 任务 | 依赖 | 核心验收 |
|---|---|---|---|---|
| TS-01 | NEXT | Workspace 隔离、Git Checkpoint、Rollback、Final Diff | TS-00 | 源仓库不变；修改仅发生在 Run Workspace；新增/修改/删除均进入 Diff；可安全回滚 |
| TS-02 | READY | Project Profile YAML 兼容加载 | TS-00 | 兼容现有 Python/Node/CMake Profile；默认值和超时语义稳定 |
| TS-03 | BLOCKED | Verification Hook 与 Finish Gate | TS-01, TS-02 | 测试失败阻止 Finish；基线失败与新增失败可区分 |
| TS-04 | BLOCKED | Trace、Result 和 Artifact 持久化 | TS-01 | 生成 trace.jsonl、result.json、verification.json、final.diff |
...
当前唯一 NEXT 任务是 TS-01：Workspace 隔离、Git Checkpoint、Rollback 和 Final Diff。

详细实施规格见：../tasks/TS-01-workspace-checkpoint.md
